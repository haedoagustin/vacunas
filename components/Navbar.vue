        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6">
            <div
                ...
                <PopoverGroup as="nav" class="hidden space-x-10 md:flex">
                    ...
                    <NuxtLink href="/" class="text-base font-medium text-gray-500 hover:text-gray-900">Inicio</NuxtLink>

                    <Popover
                        v-if="vacunas.find((item) => item.roles.includes('any') || item.roles.includes(usuario?.rol))"
                        class="relative" v-slot="{ open }">
                        <PopoverButton
                            :class="[open ? 'text-gray-900' : 'text-gray-500', 'group inline-flex items-center rounded-md bg-white text-base font-medium hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2']">
                                ...
                                <div class="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                                    <div class="relative grid gap-6 bg-white px-5 py-6 sm:gap-8 sm:p-8">
                                        <template v-for="item in vacunas" :key="item.name">
                                            <NuxtLink
                                                v-if="item.roles.includes('any') || item.roles.includes(usuario.rol)"
                                                :href="item.href"
                    ...
                    <Popover
                        v-if="compras.find((item) => item.roles.includes('any') || item.roles.includes(usuario?.rol))"
                        class="relative" v-slot="{ open }">
                        <PopoverButton
                            :class="[open ? 'text-gray-900' : 'text-gray-500', 'group inline-flex items-center rounded-md bg-white text-base font-medium hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2']">
                    ...
                    <template v-for="item in otros" :key="item.name">
                        <NuxtLink v-if="item.roles.includes('any') || item.roles.includes(usuario.rol)"
                            :href="item.href" class="text-base font-medium text-gray-500 hover:text-gray-900">{{
                                    item.name
                            }}</NuxtLink>
                    </template>
                </PopoverGroup>
