...
const meses = [...Array(12).keys()].map(key => ({ nro: ++key, nombre: new Date(0, --key).toLocaleString('es', { month: 'long' }) }))
...
const submitVacuna = async () => {
  ...
  if (vacuna.tipo.value === 'Pandemica') {
    row.fecha_inicio = vacuna.fechaInicio.value ? new Date(vacuna.fechaInicio.value) : null;
    row.fecha_fin = vacuna.fechaFin.value ? new Date(vacuna.fechaFin.value) : null;
  } else {
...
<template>
  <div class="w-full">
    <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 w-full" @submit="submitVacuna">
      <div class="mb-4 shadow-md">
        ...
        <input
          ...
          type="text" placeholder="Nombre" id="nombre" required v-model="vacuna.nombre.value" />
      </div>
      <div class="mb-4 shadow-md">
        ...
        <select
          ...
          id="tipo" placeholder="Tipo" required v-model="vacuna.tipo.value">
          <option v-for="tipo_vacuna in tipos_vacuna">{{ tipo_vacuna }}</option>
        </select>
      </div>
      <div class="mb-4 shadow-md" v-if="vacuna.tipo.value == 'Estacional'">
        <label class="block text-gray-700 text-sm font-bold mb-2" for="inicio-intervalo">
          Mes de inicio de intervalo de vacunación estacional
        </label>
        <select
          class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          id="inicio-intervalo" placeholder="inicio de intervalo" required v-model="vacuna.inicioIntervalo.value">
          <option v-for="mes in meses" :value="mes.nro">{{ mes.nombre }}</option>
        </select>
      </div>
      <div class="mb-4 shadow-md" v-if="vacuna.tipo.value == 'Estacional'">
        <label class="block text-gray-700 text-sm font-bold mb-2" for="fin-intervalo">
          Mes de fin de intervalo de vacunación estacional
        </label>
        <select
          class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          id="fin-intervalo" placeholder="Fin de intervalo" required v-model="vacuna.finIntervalo.value">
          <option v-for="mes in meses" :value="mes.nro">{{ mes.nombre }}</option>
        </select>
      </div>
      <div class="mb-4 shadow-md" v-if="vacuna.tipo.value === 'Pandemica'">
        <label class="block text-gray-700 text-sm font-bold mb-2" for="fecha-inicio">
          Fecha de inicio de pandemia
        </label>
        <input
          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          type="date" id="fecha-inicio" placeholder="fecha de inicio" v-model="vacuna.fechaInicio.value" />
      </div>
      <div class="mb-4 shadow-md" v-if="vacuna.tipo.value === 'Pandemica'">
        <label class="block text-gray-700 text-sm font-bold mb-2" for="fecha-fin">
          Fecha de fin de pandemia
        </label>
        <input
          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          type="date" id="fecha-fin" placeholder="fecha de fin" v-model="vacuna.fechaFin.value" />
      </div>
      <button href="#"
